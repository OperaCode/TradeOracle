    ...
    <div className="min-h-screen flex flex-col bg-gradient-to-br from-purple-900 via-blue-800 to-indigo-950 text-white font-sans">
      ...
      >
        ...
        <nav className="hidden md:flex space-x-6">
          ...
          <Link to="/dashboard" className="text-sm hover:text-yellow-400 transition-colors duration-200">Dashboard</Link>
      ...
      >
        <motion.h2
          ...
          className="text-3xl sm:text-4xl md:text-5xl font-bold mb pasado tracking-tight mb-4 sm:mb-6"
        >
      ...
      {/* Features Section */}
      <section id="features" className="py-12 sm:py-16 px-4 sm:px-6 bg-black/20 backdrop-blur-lg">
        <motion.h3
          initial={{ opacity: 0 }}
          ...
          {[
            {
              ...
              title: "Daily Signals",
              description: "Get clear BUY/SELL/NEUTRAL signals every day to guide your trades.",
            },
            {
              icon: <BarChart2 size={40} className="text-yellow-400" />,
              title: "Technical Indicators",
              description: "See key indicators like RSI and MACD for informed decision-making.",
            },
            {
              icon: <Shield size={40} className="text-yellow-400" />,
              title: "Simple & Secure",
              description: "Minimal, fast, and secure dashboard to streamline your trading analysis.",
            },
          ].map((feature, index) => (
            <motion.div
              key={index}
              ...
              whileHover={{ y: -5 }}
              className="flex flex-col items-center p-6 bg-white/10 rounded-xl backdrop-blur-sm hover:bg-white/20 transition-all duration-300"
            >
              <div className="mb-4">{feature.icon}</div>
              <h4 className="text-lg sm:text-xl font-semibold mb-2">{feature.title}</h4>
              <p className="text-sm sm:text-base text-center text-gray-200">{feature.description}</p>
            </motion.div>
          ))}
        </div>
      </section>
      ...
      {/* Footer */}
      <footer className="text-center py-6 sm:py-8 text-sm bg-black/30 backdrop-blur-lg">
        <p>© {new Date().getFullYear()} TradeOracle. All rights reserved.</p>
        <p className="mt-2 text-gray-300">Disclaimer: This app provides information only and is not financial advice.</p>
      </footer>
    </div>
  );
};
